...
<body>
	...
	<div id="content">
		...
		<%-- 		<jsp:include page="fragments/header.jsp"/>//без смены языка --%>
		<div id="header">
			<p><spring:message code="management" /></p>


			<c:url var="ru" value="/news/editNewsForm">
				<c:param name="id" value="${newsId.newsMessage.id}" />
				<c:param name="lang" value="ru" />
			</c:url>
			<c:url var="en" value="/news/editNewsForm">
				<c:param name="id" value="${newsId.newsMessage.id}" />
		...
		<div id="center">
			<h1><spring:message code="update" /></h1>

			<h5><i><spring:message code="requiredFieldMessage" /></i></h5>


			<!-- 	         updateNews    addNews -->
			<form:form action="updateNews" modelAttribute="newsId" method="POST">

				<form:hidden path="newsMessage.id" />
				
				<c:url var="editNewsForm" value="/news/editNewsForm">
					<c:param name="id" value="newsId.newsMessage.id" />
				</c:url>


				<h3><spring:message code="newsTitle" />*</h3>
				<form:input path="newsMessage.newsTitle" />
